...
class ModelDebug {
    ...
    ifDebugIsActive(type) {
        if (!CONFIG.debug.status) return false;
        if (type) {
            if (CONFIG.debug[`debug.${type}`] && CONFIG.debug[`debug.${type}`]['status']) return true;
            else return false;
        }
        else return true;
    }
    ...
    get title() {
        return CONFIG.debug.title;
    }

    get color() {
        return CONFIG.debug.color;
    }

    get colorOfType() {
        return CONFIG.debug[`debug.${this._type}`].color;
    }

    get titleOfType() {
        return CONFIG.debug[`debug.${this._type}`].title;
    }
